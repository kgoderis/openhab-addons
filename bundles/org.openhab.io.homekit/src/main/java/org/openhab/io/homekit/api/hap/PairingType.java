...
/**
 * Enum representing HomeKit pairing type flags as defined by Apple's HomeKit Accessory Protocol.
...
public enum PairingType {
    ...
    PairingType(int mask, int bit, String description) {
        this.mask = mask;
        this.bit = bit;
    ...
    public static boolean isSet(int flags, PairingType type) {
        return (flags & type.mask) != 0;
    }

    public static int setFlag(int flags, PairingType type) {
        return flags | type.mask;
    }

    public static int clearFlag(int flags, PairingType type) {
        return flags & ~type.mask;
    }
