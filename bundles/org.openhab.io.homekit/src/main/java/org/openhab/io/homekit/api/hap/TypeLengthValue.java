...
/**
 * Enum representing HomeKit TLV (Type-Length-Value) types as defined by Apple's HomeKit Accessory Protocol.
 */
@NonNullByDefault
public enum TypeLengthValue {
    METHOD(0x00, "Method", "Method to use for pairing"),
    ...
    FLAGS(0x13, "Flags", "Pairing Type Flags (32 bit unsigned integer)"),
    ...
    TypeLengthValue(int type, String name, String description) {
        this.type = type;
        this.name = name;
    ...
    public static TypeLengthValue fromType(int type) {
        for (TypeLengthValue tlv : values()) {
            if (tlv.type == type) {
                return tlv;
            }
        }
