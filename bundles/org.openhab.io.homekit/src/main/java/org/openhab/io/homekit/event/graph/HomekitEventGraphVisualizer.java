...
public class HomekitEventGraphVisualizer {
    private final HomekitEventGraphProcessor graph;

    public HomekitEventGraphVisualizer(HomekitEventGraphProcessor graph) {
        this.graph = graph;
    }
    ...
    public String generateDotGraph() {
        ...
        for (HomekitEventGraphProcessor.EventProcessingNode node : graph.getNodes().values()) {
        ...
        for (Map.Entry<String, Set<String>> entry : graph.getGraph().entrySet()) {
            ...
            for (String target : entry.getValue()) {
                ...
                edgeStats.addProperty("eventCount",
                        (Integer) graph.getEdgeMetadata(edgeId).getOrDefault("eventCount", 0));
                edgeStats.addProperty("averageLatency",
                        (Double) graph.getEdgeMetadata(edgeId).getOrDefault("averageLatency", 0.0));
