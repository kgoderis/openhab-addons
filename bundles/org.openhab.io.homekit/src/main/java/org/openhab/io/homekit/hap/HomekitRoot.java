...
import org.openhab.io.homekit.hap.impl.accessories.Bridge;
import org.openhab.io.homekit.hap.impl.connections.HomekitClientConnectionFactoryImpl;
...
public class HomekitRoot {
    ...
    HomekitRoot(String label, HomekitWebHandler webHandler, InetAddress localhost, HomekitAuthInfo authInfo)
            throws IOException {
        this(label, webHandler, authInfo, new JmdnsHomekitAdvertiser(localhost));
    }

    HomekitRoot(String label, HomekitWebHandler webHandler, HomekitAuthInfo authInfo, JmdnsHomekitAdvertiser advertiser)
            throws IOException {
    ...
    public void start() {
        started = true;
        registry.reset();
        webHandler.start(new HomekitClientConnectionFactoryImpl(authInfo, registry, subscriptions, advertiser))
                .thenAccept(port -> {
                    try {
                        refreshAuthInfo();
