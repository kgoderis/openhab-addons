...
public abstract class BaseCharacteristic<T> implements Characteristic {
    ...
    protected CompletableFuture<JsonObjectBuilder> makeBuilder(int instanceId) {
        ...
        return futureValue.exceptionally(t -> {
            ...
            JsonObjectBuilder builder = Json.createObjectBuilder().add("iid", instanceId).add("type", shortType)
                    .add("perms", perms.build()).add("format", format).add("ev", false).add("description", description);
