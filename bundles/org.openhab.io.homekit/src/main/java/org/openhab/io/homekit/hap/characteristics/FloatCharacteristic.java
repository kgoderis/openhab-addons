...
public abstract class FloatCharacteristic extends BaseCharacteristic<Double> {
     ...
     */
    public FloatCharacteristic(String type, boolean isWritable, boolean isReadable, String description, double minValue,
    ...
    protected CompletableFuture<JsonObjectBuilder> makeBuilder(int iid) {
        return super.makeBuilder(iid).thenApply(builder -> builder.add("minValue", minValue).add("maxValue", maxValue)
                .add("minStep", minStep).add("unit", unit));
    }
    ...
    protected final CompletableFuture<Double> getValue() {
        ...
        return getDoubleValue().thenApply(d -> d == null ? null : Math.round(d * rounder) / rounder).thenApply(d -> {
            if (d != null) {
                if (d < minValue) {
                    LOGGER.warn("Detected value out of range " + d + ". Returning min value instead. Characteristic "
                            + this);
                    return minValue;
                }
                if (d > maxValue) {
                    LOGGER.warn("Detected value out of range " + d + ". Returning max value instead. Characteristic "
                            + this);
                    return maxValue;
                }
                return d;
