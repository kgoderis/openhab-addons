...
class LengthPrefixedByteArrayProcessor {
    ...
    public synchronized Collection<byte[]> handle(byte[] data) {
        ...
        int pos = 0;
        LOGGER.debug("Received message of length {}. Existing buffer is {}", data.length, buffer.size());
        if (buffer.size() == 0) {
            while (data.length - pos > 18) {
                int targetLength = (data[0] & 0xFF) + (data[1] & 0xFF) * 256 + 16 + 2;
                LOGGER.debug("Attempting to read message of length {}", targetLength);
                if (data.length >= pos + targetLength) {
                    byte[] b = new byte[targetLength - 2];
                    System.arraycopy(data, pos + 2, b, 0, targetLength - 2);
                    results.add(b);
                    LOGGER.debug("Read complete message");
                    pos = pos + targetLength;
                } else {
                    LOGGER.debug("Not enough data available");
                    break;
                }
            }
        }
        if (data.length > pos) {
            LOGGER.debug("Remaining data available");
    ...
    private void step(byte[] data, int pos, Collection<byte[]> results) {
        LOGGER.debug("Performing step operation on buffer of length {} with pos {}", data.length, pos);
        if (targetLength == 0 && data.length == 1 + pos) {
            firstLengthByteBuffer = data[pos];
            LOGGER.debug("Received a single byte message, storing byte {} for later", firstLengthByteBuffer);
            return;
        }
        if (targetLength == 0) {
            if (firstLengthByteBuffer != null) {
                ...
                pos += 1;
                LOGGER.debug("Received the second byte after storing the first byte. New length is {}", targetLength);
            } else {
                targetLength = (data[pos] & 0xFF) + (data[pos + 1] & 0xFF) * 256 + 16;
                pos += 2;
                LOGGER.debug("targetLength is {}", targetLength);
            }
        }
        int toWrite = targetLength - buffer.size();
        if (toWrite <= data.length - pos) {
            // We have a complete message
            LOGGER.debug("Received a complete message");
            ...
            LOGGER.debug("Storing {} bytes in buffer until we receive the complete {}", data.length - pos,
                    targetLength);
