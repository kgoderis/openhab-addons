...
import org.openhab.io.homekit.hap.impl.json.EventController;
...
public class SubscriptionManager {
    ...
    public synchronized void removeSubscription(EventableCharacteristic characteristic,
            HomekitClientConnection connection) {
    ...
    public synchronized void completeUpdateBatch() {
        if (--this.nestedBatches == 0 && !pendingNotifications.isEmpty()) {
            ...
            for (ConcurrentMap.Entry<HomekitClientConnection, ArrayList<PendingNotification>> entry : pendingNotifications
                    .entrySet()) {
                try {
