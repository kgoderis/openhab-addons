 ...
 */
public class EventGraphIntegrations {
    private static final Logger logger = LoggerFactory.getLogger(EventGraphIntegrations.class);
    
    private final EventGraphProcessor eventGraph;
    private final EventGraphVisualizer eventGraphVisualizer;
    private final EventPublisher eventPublisher;
    
    public EventGraphIntegrations(EventGraphProcessor eventGraph, 
                                EventGraphVisualizer eventGraphVisualizer,
    ...
    public void logGraphState() {
        ...
        logger.info("  Nodes: {}", eventGraph.getNodes().size());
        logger.info("  Edges: {}", eventGraph.getGraph().values().stream()
            .mapToInt(Set::size).sum());
        logger.info("  Cycles: {}", eventGraph.hasCycle());
        if (eventGraph.hasCycle()) {
            logger.warn("  Cycle Path: {}", String.join(" -> ", 
                eventGraph.getCyclePath()));
        }
        
        // Log performance metrics
        Map<String, Double> performance = eventGraph.calculateNodePerformance();
        performance.forEach((nodeId, avgTime) -> 
            logger.info("  Node {} average processing time: {} ms", nodeId, avgTime));
        ...
        if (eventPublisher != null) {
            EventSubscriber subscriber = new EventSubscriber() {
                ...
                public void receive(Event event) {
                    if (event instanceof EventGraphUpdatedEvent) {
                        EventGraphUpdatedEvent graphEvent = (EventGraphUpdatedEvent) event;
                        eventGraph.addNode(graphEvent.getNodeId());
                        eventGraph.addEdge(graphEvent.getNodeId(), graphEvent.getEdgeId());
                        
                        if (eventGraph.hasCycle()) {
                            eventPublisher.post(new EventCycleDetectedEvent(
                                eventGraph.getCyclePath()));
                        }
                    }
                }
                ...
                public Set<String> getSubscribedEventTypes() {
                    return Set.of(EventGraphUpdatedEvent.TYPE);
                }
            };
            
            // Register the subscriber using the appropriate method
            try {
                eventPublisher.getClass().getMethod("addEventSubscriber", EventSubscriber.class)
                    .invoke(eventPublisher, subscriber);
            } catch (Exception e) {
