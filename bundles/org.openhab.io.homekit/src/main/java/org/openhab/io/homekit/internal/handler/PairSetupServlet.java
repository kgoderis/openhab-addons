...
import org.openhab.io.homekit.internal.handler.HomekitSRP6ServerSession.State;
...
public class PairSetupServlet extends BaseServlet {
    ...
    protected void doStage1(HttpServletRequest request, HttpServletResponse response, byte[] body)
        ...
        if (SRP6Session.getState() != State.INIT) {
            ...
            logger.info("Stage 1 : Encoder : Add {} : {} ", Message.SALT,
                    byteToHexString(bigIntegerToUnsignedByteArray(server.getSalt())));
            encoder.add(Message.SALT, server.getSalt());
            BigInteger publicKey = SRP6Session.step1(IDENTIFIER, server.getSalt(), verifier);
            logger.info("Stage 1 : Encoder : Add {} : {} ", Message.PUBLIC_KEY,
                    byteToHexString(bigIntegerToUnsignedByteArray(publicKey)));
            encoder.add(Message.PUBLIC_KEY, publicKey);

            // Chunked transfer encoding is for data of undetermined size. It is more correct, better performant, and
            // much better supported than the fallback of data without a content-length. Historically, this logic for
            // chunked encoding has existed since Jetty 4.2.9 (I know, this surprised me too, but google backs me up on
            // this)
            //
            // If the servlet doesn't give a Content-Length and just starts to write content out via either the
            // HtttpServletResponse.getOutputStream() or HttpServletResponse.getWriter() then the HTTP response is
            // chunked.
            //
            // As for disabling chunked encoding, to do that, you must provide the content length via
            // HttpServletResponse.setContentLength(long), before you access the HttpServletResponse.getOutputStream()
            // and HttpServletResponse.getWriter().

            //
            // You should be able to disable chunking by disabling the persistent connection itself by adding a
            // Connection: close header to the response,
            //
        ...
        if (SRP6Session == null) {
            ...
            if (SRP6Session.getState() != State.STEP_1) {
                ...
                try {
                    ...
                    logger.info("Stage 2 : Encoder : Add {} : {} ", Message.STATE, (short) 0x04);
                    encoder.add(Message.STATE, (short) 0x04);
                    logger.info("Stage 2 : Encoder : Add {} : {} ", Message.PROOF, proof);
    ...
    protected void doStage3(HttpServletRequest request, HttpServletResponse response, byte[] body)
        ...
        if (SRP6Session == null) {
            ...
            if (reportError) {
                ...
                logger.info("Stage 3 : Server Pairing Id is {}", server.getId());
                encoder.add(Message.IDENTIFIER, server.getId().getBytes(StandardCharsets.UTF_8));
                logger.info("Stage 3 : Encoder : Add {} : {} ", Message.IDENTIFIER,
                        byteToHexString(server.getId().getBytes(StandardCharsets.UTF_8)));

                encoder.add(Message.PUBLIC_KEY, signer.getPublicKey());
                logger.info("Stage 3 : Encoder : Add {} : {} ", Message.PUBLIC_KEY,
                        byteToHexString(signer.getPublicKey()));

                encoder.add(Message.SIGNATURE, accessorySignature);
                logger.info("Stage 3 : Encoder : Add {} : {} ", Message.SIGNATURE, byteToHexString(accessorySignature));
                ...
                encoder = TypeLengthValue.getEncoder();
                encoder.add(Message.STATE, (short) 6);
                logger.info("Stage 3 : Encoder : Add {} : {} ", Message.STATE, (short) 6);
                encoder.add(Message.ENCRYPTED_DATA, ciphertext);
                logger.info("Stage 3 : Encoder : Add {} : {} ", Message.ENCRYPTED_DATA, byteToHexString(ciphertext));
