...
public class HomekitHttpParser {
    ...
    static {
        ...
        // Add headers with null values so HomekitHttpParser can avoid looking up name again for unknown values
    ...
    @Deprecated
    public HomekitHttpParser(RequestHandler handler, int maxHeaderBytes, boolean strict) {
        this(handler, maxHeaderBytes, strict ? HttpCompliance.LEGACY : compliance());
    }

    @Deprecated
    public HomekitHttpParser(ResponseHandler handler, int maxHeaderBytes, boolean strict) {
        this(handler, maxHeaderBytes, strict ? HttpCompliance.LEGACY : compliance());
    }

    public HomekitHttpParser(RequestHandler handler, HttpCompliance compliance) {
        this(handler, -1, compliance);
    }
    ...
    private boolean quickStart(ByteBuffer buffer) {
        if (_requestHandler != null) {
            ...
            _version = HomekitHttpVersion.lookAheadGet(buffer);
            if (_version != null) {
                        ...
                        case COLON:
                            ...
                            if (_responseHandler != null) {
                                ...
                                if (buffer.position() > 0 && buffer.hasArray()) {
                                    ...
                                    version = HomekitHttpVersion.getBest(buffer, 0, buffer.remaining());
                                }
                ...
                case REQUEST_VERSION:
                    switch (t.getType()) {
                        case LF:
                            if (_version == null) {
                                _length = _string.length();
                                _version = HomekitHttpVersion.get(takeString());
                            }
                            checkVersion();
            ...
            if (_header != null) {
                ...
                switch (_header) {
                    ...
                    case TRANSFER_ENCODING:
                        ...
                        if (_endOfContent == EndOfContent.CHUNKED_CONTENT) {
                            throw new BadMessageException(HttpStatus.BAD_REQUEST_400,
                                    "Bad Transfer-Encoding, chunked not last");
                        }
                        ...
                        if (HttpHeaderValue.CHUNKED.is(_valueString)) {
                            ...
                            for (int i = 0; i < len; i++) {
                                if (HttpHeaderValue.CHUNKED.is(values.get(i))) {
                                    if (chunked != -1) {
                                        throw new BadMessageException(HttpStatus.BAD_REQUEST_400,
                                                "Bad Transfer-Encoding, multiple chunked tokens");
                                    }
                                    chunked = i;
                                    // declared chunked
                                    _endOfContent = EndOfContent.CHUNKED_CONTENT;
                    ...
                    case CONNECTION:
                        ...
                        if (HttpHeaderValue.CLOSE.is(_valueString) || new QuotedCSV(_valueString).getValues().stream()
                                .anyMatch(HttpHeaderValue.CLOSE::is)) {
                            _fieldCache = null;
                        }
                        break;
    ...
    public String toString() {
        return String.format("%s{s=%s,%d of %d}", getClass().getSimpleName(), _state, getContentRead(),
                getContentLength());
    }
    ...
    public interface RequestHandler extends HttpHandler {
         ...
         * @param uri The raw bytes of the URI. These are copied into a ByteBuffer that will not be changed until this
