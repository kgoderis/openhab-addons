...
public abstract class HttpComponentConfigurationAdapter<COMP, CONFIG extends ConfigAdapter> {
    ...
    protected final AccessoryServer server;

    public HttpComponentConfigurationAdapter(AccessoryServer server, Class<? extends COMP> componentClazz) {
        this(server, Utils.newInstance(componentClazz), DEFAULT_URL_PATTERN);
    }

    public HttpComponentConfigurationAdapter(AccessoryServer server, Class<? extends COMP> servletClazz,
            String... urlPatterns) {
        this(server, Utils.newInstance(servletClazz), urlPatterns);
    }

    public HttpComponentConfigurationAdapter(AccessoryServer server, COMP servlet) {
        this(server, servlet, DEFAULT_URL_PATTERN);
    }

    @SuppressWarnings("unchecked")
    public HttpComponentConfigurationAdapter(AccessoryServer server, COMP component, String... urlPatterns) {
        ...
        this.config = newConfigInstance((Class<? extends COMP>) component.getClass());
    }

    protected abstract CONFIG newConfigInstance(Class<? extends COMP> componentClazz);
