...
import org.openhab.io.homekit.internal.handler.AccessoryServlet;
import org.openhab.io.homekit.internal.handler.CharacteristicServlet;
import org.openhab.io.homekit.internal.handler.PairSetupServlet;
import org.openhab.io.homekit.internal.handler.PairVerificationServlet;
import org.openhab.io.homekit.internal.handler.PairingServlet;
...
public abstract class AbstractAccessoryServer implements AccessoryServer {
    ...
    public AbstractAccessoryServer(InetAddress localAddress, int port, String pairingId, BigInteger salt,
        ...
        http.setPort(port);
        // http.setHost(localAddress.getHostName());
        ...
        // HomekitContextListener contextListener = new HomekitContextListener();
        ...
        // new HomekitSessionTracker(servletContextHandler.getServer(), sessionHandler);
        ...
        // ServletHolder defaultHolder = new ServletHolder(new CatchAnyServlet(this));
        // servletContextHandler.addServlet(defaultHolder, "/");

        // servletContextHandler.addFilter(EncryptionFilter.class, "/accessories",
        // EnumSet.of(DispatcherType.INCLUDE, DispatcherType.REQUEST));
        // servletContextHandler.addFilter(EncryptionFilter.class, "/characteristics",
        // EnumSet.of(DispatcherType.INCLUDE, DispatcherType.REQUEST));
        // servletContextHandler.addFilter(EncryptionFilter.class, "/",
        // EnumSet.of(DispatcherType.INCLUDE, DispatcherType.REQUEST));

        // OLD CODE

        // pairSetupHolder.setServletHandler(pairSetupHandler);

        // HandlerList pairVerificationHandlers = new HandlerList();
        // pairVerificationHandlers.addHandler(new PairVerificationStageOneHandler(this));
        // pairVerificationHandlers.addHandler(new PairVerificationStageTwoHandler(this));
        //
        // ServletHandler pairVerificationHandler = new ServletHandler();
        // pairVerificationHandler.setHandler(pairVerificationHandlers);
        // ServletHolder pairVerificationHolder = new ServletHolder(HttpServlet.class);
        // pairVerificationHolder.setServletHandler(pairVerificationHandler);
        //
        // ServletHolder accessoryHolder = new ServletHolder(HttpServlet.class);
        // accessoryHolder.setServletHandler(new AccessoryHandler(this));
        //
        // ServletHolder characteristicsHolder = new ServletHolder(HttpServlet.class);
        // characteristicsHolder.setServletHandler(new CharacteristicHandler(this));
        //

        // ServletHolder pairSetupServletHolder = servletContextHandler
        // .addServlet(org.glassfish.jersey.servlet.ServletContainer.class, "/pair-setup");

        // pairSetupServletHolder.setInitOrder(0);
        // pairSetupServletHolder.setServletHandler(pairSetupHandler);
        ...
        // ContextHandler pairSetupContextHandler = new ContextHandler();
        // pairSetupContextHandler.setContextPath("/pair-setup");
        // pairSetupContextHandler.setBaseResource(baseResource1);
        // pairSetupContextHandler.setHandler(pairSetupHandlers);
        // contextHandlers.addHandler(pairSetupContextHandler);

        // ContextHandler pairVerifyContextHandler = new ContextHandler();
        // pairVerifyContextHandler.setContextPath("/pair-verify");
        // pairVerifyContextHandler.setBaseResource(baseResource1);
        // pairVerifyContextHandler.setHandler(pairVerificationHandlers);
        // contextHandlers.addHandler(pairVerifyContextHandler);

    }

    // TODO : Add this to builder instead of doing via constructor?
    ...
    public Collection<Accessory> getAccessories() {
        return accessoryRegistry.get(pairingIdentifier);
    }

    @Override
    public @Nullable Accessory getAccessory(int instanceId) {
        AccessoryUID id = new AccessoryUID(pairingIdentifier, Integer.toString(instanceId));
        return accessoryRegistry.get(id);
    }

    @Override
    public @Nullable Accessory getAccessory(Class<? extends Accessory> accessoryClass) {
        Collection<Accessory> accessories = accessoryRegistry.get(pairingIdentifier);
    ...
    public void addNotification(Characteristic<?> characteristic, HttpConnection connection) {
        notificationRegistry.add(new NotificationImpl(characteristic, connection));
    }
    ...
    public synchronized void advertise() {
        ...
        props.put("id", pairingIdentifier);
        ...
        try {
            ...
            if (oldPairing != null) {
                logger.debug("Removed Pairing of Server {} with Client {} and Public Key {}", getId(),
                        oldPairing.getClientPairingId(), oldPairing.getClientLongtermPublicKey());
            }
