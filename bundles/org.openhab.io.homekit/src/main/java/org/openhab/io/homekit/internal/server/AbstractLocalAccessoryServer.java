...
public abstract class AbstractLocalAccessoryServer extends AbstractAccessoryServer {
    ...
    public AbstractLocalAccessoryServer(InetAddress address, int port, byte[] pairingId, byte[] secretKey,
            ...
            NotificationRegistry notificationRegistry,  SafeCaller safeCaller)
    ...
    protected synchronized void removeConnectionMapping(Characteristic<?> characteristic, HttpConnection connection) {
        ...
        if (connections != null) {
            ...
            logger.debug("Removed connection mapping for characteristic {} from connection {}", characteristic, connection);
        }
    }
    ...
    protected synchronized void publish(HttpConnection connection, JsonArrayBuilder arrayBuilder) {
    ...
    public long getNextAvailableAccessoryId() {
        try {
            long maxInstanceIdInRegistry = accessoryRegistry.get(this.getAccessoryId()).stream().map(a -> a.getAccessoryId())
