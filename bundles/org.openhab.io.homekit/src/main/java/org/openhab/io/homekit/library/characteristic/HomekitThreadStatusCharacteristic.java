...
public class HomekitThreadStatusCharacteristic extends HomekitTLV8Characteristic {
    public HomekitThreadStatusCharacteristic(HomekitService service, HomekitEventManager eventManager, long instanceId) {
        super(service, eventManager);
        withInstanceId(instanceId)
            .withPairedRead(true)
            .withPairedWrite(false)
            .withEvents(true)
            .withDescription("Thread Status");
    }

    public HomekitThreadStatusCharacteristic(HomekitService service, HomekitEventManager eventManager, JsonValue value) {
