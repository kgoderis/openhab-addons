...
/**
 * HomeKit Light Sensor Service.
 * <p>
 * This service provides monitoring of ambient light levels in HomeKit accessories.
 * It enables tracking of light measurements and sensor status information.
 * </p>
 *
 * <ul>
 *   <li>Ambient light level monitoring</li>
 ...
 * <ul>
 *   <li>CurrentAmbientLightLevel - Current ambient light level in lux (0.0001 to 100000)</li>
 * </ul>
 * </p>
 *
 * <p>
...
public class HomekitLightSensorService extends AbstractHomekitService {
    ...
    private static final String LOG_PREFIX = "Homekit LightSensorService: ";
    ...
    private final Logger logger = LoggerFactory.getLogger(HomekitLightSensorService.class);
    ...
    public HomekitLightSensorService(HomekitAccessory accessory, HomekitEventManager eventManager,
        ...
        logger.debug("{}Created LightSensorService for accessory {}", LOG_INIT, accessory.getLabel());
    }

    /**
     * Creates a new Light Sensor service from a JSON configuration.
     *
     * @param accessory The accessory this service belongs to
     * @param eventManager The event manager for handling HomeKit events
     * @param characteristicFactory Factory for creating HomeKit characteristics
    ...
    public HomekitLightSensorService(HomekitAccessory accessory, HomekitEventManager eventManager,
        ...
        logger.debug("{}Created LightSensorService from JSON for accessory {}", LOG_INIT, accessory.getLabel());
    }
    ...
    public void addCharacteristics() {
        logger.trace("{}Adding required characteristics to LightSensorService for accessory {}", LOG_TRACE, getAccessory().getLabel());
        
        addCharacteristic(new HomekitCurrentAmbientLightLevelCharacteristic(this, eventManager,
                getAccessory().getNextAvailableInstanceId()).withMandatory(true));
        logger.debug("{}Added CurrentAmbientLightLevelCharacteristic to LightSensorService", LOG_STATE);
        
        addCharacteristic(new HomekitNameCharacteristic(this, eventManager, getAccessory().getNextAvailableInstanceId())
                .withMandatory(false));
        logger.debug("{}Added NameCharacteristic to LightSensorService", LOG_STATE);
        
        addCharacteristic(
                new HomekitStatusActiveCharacteristic(this, eventManager, getAccessory().getNextAvailableInstanceId())
                        .withMandatory(false));
        logger.debug("{}Added StatusActiveCharacteristic to LightSensorService", LOG_STATE);
        
        addCharacteristic(
                new HomekitStatusFaultCharacteristic(this, eventManager, getAccessory().getNextAvailableInstanceId())
                        .withMandatory(false));
        logger.debug("{}Added StatusFaultCharacteristic to LightSensorService", LOG_STATE);
        
        addCharacteristic(new HomekitStatusLowBatteryCharacteristic(this, eventManager,
                getAccessory().getNextAvailableInstanceId()).withMandatory(false));
        logger.debug("{}Added StatusLowBatteryCharacteristic to LightSensorService", LOG_STATE);
        
        addCharacteristic(
                new HomekitStatusTamperedCharacteristic(this, eventManager, getAccessory().getNextAvailableInstanceId())
                        .withMandatory(false));
        logger.debug("{}Added StatusTamperedCharacteristic to LightSensorService", LOG_STATE);
    }

    /**
     * Indicates that this service is not extensible.
     * Light sensor service has a fixed set of characteristics.
     *
