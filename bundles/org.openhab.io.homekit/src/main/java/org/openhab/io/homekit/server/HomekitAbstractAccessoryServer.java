...
public abstract class HomekitAbstractAccessoryServer implements HomekitAccessoryServer, AutoCloseable {
    ...
    public HomekitAbstractAccessoryServer(HomekitAccessoryCategory category, String serverId, InetAddress address,
        ...
        logger.debug("{}[{}]: {} - Homekit server initialization completed", LOG_PREFIX, getServerId(), LOG_INIT);
    }
    ...
    private void cleanupAccessories() {
        ...
        for (HomekitAccessory accessory : accessoriesToRemove) {
            try {
                ...
                logger.error("{}[{}]: {} - Failed to remove accessory during shutdown: {}", LOG_PREFIX, getServerId(),
    ...
    public void start() throws HomekitServerException {
        synchronized (stateLock) {
            ...
            if (currentState == HomekitAccessoryServerState.READY) {
                logger.warn("{}[{}]: {} - Server is already running", LOG_PREFIX, getServerId(), LOG_SERVER);
                return;
            }
            if (currentState == HomekitAccessoryServerState.STOPPED) {
                logger.warn("{}[{}]: {} - Server is in stopped state, attempting to restart", LOG_PREFIX, getServerId(),
                        LOG_SERVER);
            }
            ...
            try {
                ...
                logger.info("{}[{}]: {} - Server started successfully", LOG_PREFIX, getServerId(), LOG_SERVER);
            } catch (Exception e) {
                try {
                    cleanupResources();
                } catch (Exception cleanupEx) {
                    logger.error("{}[{}]: {} - Failed to cleanup resources after failed start: {}", LOG_PREFIX,
                            getServerId(), LOG_ERROR, cleanupEx.getMessage(), cleanupEx);
                }
    ...
    public void stop() throws HomekitServerException {
        synchronized (stateLock) {
            if (currentState == HomekitAccessoryServerState.STOPPED) {
                logger.warn("{}[{}]: {} - Server is already stopped", LOG_PREFIX, getServerId(), LOG_SERVER);
                return;
            }
            try {
                try {
                    ...
                    logger.error("{}[{}]: {} - Failed to set stopped state during shutdown: {}", LOG_PREFIX,
                            getServerId(), LOG_ERROR, e.getMessage(), e);
                    throw new HomekitServerException("Failed to set stopped state during shutdown", e);
                }
            } catch (HomekitServerException e) {
                logger.error("{}[{}]: {} - Failed to stop server gracefully: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                        e.getMessage(), e);
                forceStop();
            }
        }
    ...
    private void forceStop() {
        try {
            cleanupResources();
            setState(HomekitAccessoryServerState.STOPPED);
            logger.warn("{}[{}]: {} - Server force stopped", LOG_PREFIX, getServerId(), LOG_WARN);
        } catch (Exception e) {
            logger.error("{}[{}]: {} - Failed to force stop server: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                    e.getMessage(), e);
        }
    }
    ...
    protected synchronized void setState(HomekitAccessoryServerState newState) throws HomekitServerException {
        try {
            ...
            logger.debug("{}[{}]: {} - Server state changed from {} to {}", LOG_PREFIX, getServerId(), LOG_STATE,
                    oldState, newState);
            ...
            logger.error("{}[{}]: {} - Invalid state transition: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                    e.getMessage());
    ...
    public void setConfigurationIndex(int newIndex) throws HomekitConfigurationException {
        ...
        logger.debug("{}[{}]: {} - Configuration index updated to {}", LOG_PREFIX, getServerId(), LOG_CONFIG, newIndex);
    ...
    protected synchronized void incrementConfigurationIndex() {
        configurationIndex++;
        logger.debug("{}[{}]: {} - Configuration index incremented to {}", LOG_PREFIX, getServerId(), LOG_CONFIG,
                configurationIndex);
        eventManager.publishEvent(
                new HomekitAccessoryServerEvent(HomekitEventType.SERVER_STATE_CONFIGURATION_NUMBER_CHANGED, this,
                        (HomekitAccessory) null, (HomekitService) null, (HomekitCharacteristic<?>) null));
    ...
    public void factoryReset() {
        logger.info("{}[{}]: {} - Performing factory reset", LOG_PREFIX, getServerId(), LOG_CONFIG);
        try {
            ...
            eventManager.publishEvent(new HomekitAccessoryServerEvent(HomekitEventType.SERVER_STATE_UNPAIRED, this,
                    (HomekitAccessory) null, (HomekitService) null, (HomekitCharacteristic<?>) null));
            logger.info("{}[{}]: {} - Factory reset completed successfully", LOG_PREFIX, getServerId(), LOG_CONFIG);
        } catch (Exception e) {
            logger.error("{}[{}]: {} - Failed to perform factory reset: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                    e.getMessage(), e);
        }
    }
    ...
    public InetAddress getAddress() {
        logger.trace("{}[{}]: {} - Getting server address: {}", LOG_PREFIX, getServerId(), LOG_CONFIG, address);
        return address;
    }
    ...
    @Override
    public int getPort() {
        logger.trace("{}[{}]: {} - Getting server port: {}", LOG_PREFIX, getServerId(), LOG_CONFIG, port);
        return port;
    }
    ...
    public byte[] getPairingId() {
        logger.trace("{}[{}]: {} - Getting pairing ID", LOG_PREFIX, getServerId(), LOG_CONFIG);
    ...
    public byte[] getSecretKey() {
        logger.trace("{}[{}]: {} - Getting secret key", LOG_PREFIX, getServerId(), LOG_CONFIG);
        return secretKey;
    }
    ...
    public Optional<byte[]> getPublicKey(byte[] destinationPairingId) {
        logger.debug("{}[{}]: {} - Getting public key for destination pairing ID: {}", LOG_PREFIX, getServerId(),
    ...
    public String getSetupCode() {
        logger.trace("{}[{}]: {} - Getting setup code", LOG_PREFIX, getServerId(), LOG_CONFIG);
        return setupCode;
    }
    ...
    public void addPairing(byte[] pairingId, byte[] publicKey) throws HomekitServerException {
        ...
        if (pairingRegistry.get(new HomekitPairingUIDImpl(getPairingId(), pairingId)) != null) {
            ...
            logger.error("{}[{}]: {} - HomekitPairing validation error: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                    error);
            throw new HomekitServerException(error);
        }

        logger.debug("{}[{}]: {} - Adding pairing - ID: {}", LOG_PREFIX, getServerId(), LOG_PAIRING,
        ...
        try {
            ...
            if (oldPairing != null) {
                logger.debug("{}[{}]: {} - Removed existing pairing - Destination: {}, Public Key: {}", LOG_PREFIX,
                        getServerId(), LOG_PAIRING, HomekitByte.toHexString(oldPairing.getDestinationId()),
            ...
            logger.debug("{}[{}]: {} - HomekitPairing added successfully", LOG_PREFIX, getServerId(), LOG_PAIRING);
            setState(HomekitAccessoryServerState.PAIRED);
            logger.info("{}[{}]: {} - HomekitPairing added successfully - ID: {}", LOG_PREFIX, getServerId(),
            ...
            String error = String.format("Failed to add pairing %s: %s", HomekitByte.toHexString(pairingId),
                    e.getMessage());
            logger.error("{}[{}]: {} - HomekitPairing addition error: {}", LOG_PREFIX, getServerId(), LOG_ERROR, error,
                    e);
    ...
    public Collection<HomekitPairing> getPairings() {
        logger.debug("{}[{}]: {} - Getting all pairings", LOG_PREFIX, getServerId(), LOG_PAIRING);
    ...
    public void removePairing(byte[] pairingId) throws HomekitServerException {
        ...
        logger.debug("{}[{}]: {} - Removing pairing - ID: {}", LOG_PREFIX, getServerId(), LOG_PAIRING,
                HomekitByte.toHexString(pairingId));
        ...
        try {
            ...
            if (pairingRegistry.remove(uid) != null) {
                ...
                logger.info("{}[{}]: {} - HomekitPairing removed successfully - ID: {}", LOG_PREFIX, getServerId(),
                        LOG_PAIRING, HomekitByte.toHexString(pairingId));
            }
        } catch (HomekitServerException e) {
            String error = String.format("Failed to remove pairing %s: %s", HomekitByte.toHexString(pairingId),
                    e.getMessage());
            logger.error("{}[{}]: {} - HomekitPairing removal error: {}", LOG_PREFIX, getServerId(), LOG_ERROR, error,
                    e);
            throw new HomekitServerException(error, e);
        }
    }
    ...
    protected void handlePairingVerification(boolean verified) throws HomekitServerException {
        logger.debug("{}[{}]: {} - Handling pairing verification - Verified: {}", LOG_PREFIX, getServerId(), LOG_STATE,
                verified);
        synchronized (stateLock) {
            if (verified) {
                setState(HomekitAccessoryServerState.PAIR_VERIFIED);
                logger.info("{}[{}]: {} - HomekitPairing verified successfully", LOG_PREFIX, getServerId(), LOG_STATE);
            } else {
                setState(HomekitAccessoryServerState.PAIRED);
                logger.info("{}[{}]: {} - HomekitPairing verification failed", LOG_PREFIX, getServerId(), LOG_STATE);
            }
        }
    }
    ...
    public boolean isPaired() {
        synchronized (stateLock) {
            ...
            try {
                ...
                logger.error("{}[{}]: {} - Failed to update pairing state: {}", LOG_PREFIX, getServerId(), LOG_ERROR,
                        e.getMessage(), e);
    ...
    public void addAccessory(HomekitAccessory accessory) throws HomekitAccessoryOperationException {
        ...
        synchronized (accessoryLock) {
            logger.debug("{}[{}]: {} - Adding accessory - UID: {}, Type: {}, Server: {}", LOG_PREFIX, getServerId(),
            ...
            if (accessories.add(accessory)) {
                try {
                    ...
                    logger.info("{}[{}]: {} - HomekitAccessory added successfully - UID: {}", LOG_PREFIX, getServerId(),
                    ...
                    logger.error("{}[{}]: {} - Failed to add accessory due to runtime error - UID: {}, Error: {}",
                            LOG_PREFIX, getServerId(), LOG_ERROR, accessory.getUID(), e.getMessage());
                    ...
                    // Rollback on failure
                    accessories.remove(accessory);
                    logger.error("{}[{}]: {} - Failed to add accessory - UID: {}, Error: {}", LOG_PREFIX, getServerId(),
                            LOG_ERROR, accessory.getUID(), e.getMessage());
                    throw new HomekitAccessoryOperationException(
                            String.format("Failed to add accessory %s: %s", accessory.getUID(), e.getMessage()), e);
                }
            } else {
                logger.warn("{}[{}]: {} - HomekitAccessory already exists - UID: {}", LOG_PREFIX, getServerId(),
                        LOG_WARN, accessory.getUID());
                throw new HomekitAccessoryOperationException(
    ...
    public void removeAccessory(HomekitAccessory accessory) throws HomekitAccessoryOperationException {
        ...
        synchronized (accessoryLock) {
            logger.debug("{}[{}]: {} - Removing accessory - UID: {}, Type: {}, Server: {}", LOG_PREFIX, getServerId(),
                    LOG_ACCESSORY, accessory.getUID(), accessory.getClass().getSimpleName(), this.getUID());
            ...
            if (accessories.remove(accessory)) {
                try {
                    ...
                    eventManager.publishEvent(new HomekitAccessoryServerEvent(HomekitEventType.ACCESSORY_REMOVED, this,
                            accessory, (HomekitService) null, (HomekitCharacteristic<?>) null));
                    logger.info("{}[{}]: {} - HomekitAccessory removed successfully - UID: {}", LOG_PREFIX,
                            getServerId(), LOG_ACCESSORY, accessory.getUID());
                } catch (RuntimeException e) {
                    // Rollback on failure
                    ...
                    // Rollback on failure
                    accessories.add(accessory);
                    logger.error("{}[{}]: {} - Failed to remove accessory - UID: {}, Error: {}", LOG_PREFIX,
                            getServerId(), LOG_ERROR, accessory.getUID(), e.getMessage());
                    throw new HomekitAccessoryOperationException(
                            String.format("Failed to remove accessory %s: %s", accessory.getUID(), e.getMessage()), e);
                }

            } else {
                logger.warn("{}[{}]: {} - HomekitAccessory not found - UID: {}", LOG_PREFIX, getServerId(), LOG_WARN,
                        accessory.getUID());
                throw new HomekitAccessoryOperationException(
    ...
    public boolean isBridge() {
        logger.debug("{}[{}]: {} - Checking if server is a bridge: {}", LOG_PREFIX, getServerId(), LOG_CONFIG,
    ...
    protected void handleConnection(boolean connected) throws HomekitServerException {
        logger.debug("{}[{}]: {} - Handling connection - Connected: {}", LOG_PREFIX, getServerId(), LOG_STATE,
                connected);
        synchronized (stateLock) {
            if (connected) {
                setState(HomekitAccessoryServerState.CONNECTED);
                logger.info("{}[{}]: {} - Connection established", LOG_PREFIX, getServerId(), LOG_STATE);
            } else {
                setState(HomekitAccessoryServerState.DISCONNECTED);
                logger.info("{}[{}]: {} - Connection terminated", LOG_PREFIX, getServerId(), LOG_STATE);
            }
        }
    }
    ...
    public void close() throws Exception {
        logger.info("{}[{}]: {} - Closing Homekit accessory server", LOG_PREFIX, getServerId(), LOG_INIT);
        synchronized (stateLock) {
            if (!isShutdown) {
                try {
                    stop();
                    ...
                    cleanupResources();
                    isShutdown = true;
                    logger.info("{}[{}]: {} - Homekit accessory server closed successfully", LOG_PREFIX, getServerId(),
                            LOG_INIT);
                } catch (Exception e) {
                ...
                logger.warn("{}[{}]: {} - Homekit accessory server is already closed", LOG_PREFIX, getServerId(),
                        LOG_WARN);
            }
        }
    }
